...
export class LocalStorageService {
  ...
  get(key: string): any {
    try {
      // do not to return undefined because JSON.parse doesn't work it
  ...
  clearTokens(): void {
    Object.keys(memoryStorage).forEach(key => {
      key.match(/token/i) && delete memoryStorage[key];
    });
    Object.keys(window.localStorage).forEach(key => {
      key.match(/token/i) && delete window.localStorage[key];
    });
  }
